...
function officialMatchSnapshot(match) {
  ...
  const hasLoggedScore = home.Score !== null && home.Score !== undefined && away.Score !== null && away.Score !== undefined;
  const isFinal = hasLoggedScore && match.MatchStatus === 0;
  return {
    ...
    officialLineupStatus:
      isFinal
        ? `FIFA final score logged: ${home.Score}-${away.Score}.`
...
function mergeOfficial(seedMatch, official) {
  const finalReadiness =
    ...
    official.homeScore !== undefined &&
    official.awayScore !== null &&
    official.awayScore !== undefined &&
    official.matchStatus === 0
      ? `Final: FIFA logged a ${official.homeScore}-${official.awayScore} result; pre-match probability archived.`
      : null;
  return {
    ...seedMatch,
    official: official || {
...
function formatRefereeList(referees = []) {
  return referees.length > 0 ? referees.join(", ") : "referee pending";
}

function tacticsLine(official) {
  if (!official.homeTactics && !official.awayTactics) return "";
  return `; tactics visible as ${official.home} ${official.homeTactics || "pending"} and ${official.away} ${official.awayTactics || "pending"}`;
}

function statusLine(official) {
  if (!official) return null;
  if (official.homeScore !== null && official.awayScore !== null && official.matchStatus === 0) {
    return `Match ${official.matchNo} ${official.home} vs ${official.away} is logged ${official.homeScore}-${official.awayScore} final${official.matchTime ? ` at ${official.matchTime}` : ""}${tacticsLine(official)}.`;
  }
  const status = official.matchTime ? `${official.matchTime} match time` : "pre-match status";
  return `Match ${official.matchNo} ${official.home} vs ${official.away} shows ${status}, referee ${formatRefereeList(official.referees)}${tacticsLine(official)}; starter-name fields are not exposed.`;
}

function refreshSources(sourceList, matches, now, sourceError) {
  const checkedAtEastern = easternTimeLabel(now);
  return sourceList.map((source) => {
    if (source.label === "FIFA calendar API") {
      if (sourceError) {
        return {
          ...source,
          status: `Refresh attempted ${checkedAtEastern}; FIFA calendar returned an error: ${sourceError}.`
        };
      }
      return {
        ...source,
        status: `Refreshed ${checkedAtEastern}; ${matches
          .map((match) => statusLine(match.official))
          .filter(Boolean)
          .join(" ")}`
      };
    }

    if (source.label === "FIFA match centre") {
      const match = matches.find((candidate) => source.url.includes(candidate.fifaMatchId));
      if (!match || !match.official || match.official.sourceError) return source;
      return {
        ...source,
        status: `Latest FIFA-backed fields: ${statusLine(match.official)}`
      };
    }
...
async function buildMatchdayPayload(options = {}) {
  ...
  return {
    ...
    sources: refreshSources(seed.sources, matches, now, sourceError),
    matches
  };
}
