...
/* ---- live Elo auto-refresh from eloratings.net World.tsv ----
   Keeps ratings current every render (effectively daily+) so the static elo.json never drifts.
   The static file remains the fallback; a live value is only trusted when it is within +/-200 of
   the vetted static rating (guards against eloratings reassigning a code, or a parse glitch). */
const ELO_CODE = {
  "Algeria":"DZ","Argentina":"AR","Australia":"AU","Austria":"AT","Belgium":"BE",
  "Bosnia and Herzegovina":"BA","Brazil":"BR","Cabo Verde":"CV","Canada":"CA","Colombia":"CO",
  "Congo DR":"CD","Croatia":"HR","Curaçao":"CW","Czechia":"CZ","Côte d'Ivoire":"CI",
  "Ecuador":"EC","Egypt":"EG","England":"EN","France":"FR","Germany":"DE","Ghana":"GH",
  "Haiti":"HT","IR Iran":"IR","Iraq":"IQ","Japan":"JP","Jordan":"JO","Korea Republic":"KR",
  "Mexico":"MX","Morocco":"MA","Netherlands":"NL","New Zealand":"NZ","Norway":"NO","Panama":"PA",
  "Paraguay":"PY","Portugal":"PT","Qatar":"QA","Saudi Arabia":"SA","Scotland":"SQ","Senegal":"SN",
  "South Africa":"ZA","Spain":"ES","Sweden":"SE","Switzerland":"CH","Tunisia":"TN","Turkiye":"TR",
  "United States":"US","Uruguay":"UY","Uzbekistan":"UZ"
};
const ELO_GUARD = 200;                    // max accepted gap between live and vetted static value
const LIVE_ELO_TTL = 6 * 60 * 60 * 1000;  // re-pull at most every 6h per warm instance
let _liveElo = null, _liveEloAt = 0;
async function getLiveElo(){
  if (_liveElo && (Date.now() - _liveEloAt) < LIVE_ELO_TTL) return _liveElo;
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 6000);
    const res = await fetch("https://www.eloratings.net/World.tsv", { signal: ctrl.signal });
    clearTimeout(t);
    ...
    txt.split("\n").forEach(line => {
      const c = line.split("\t");
      if (c.length > 4) { const r = parseInt(c[3], 10); if (c[2] && Number.isFinite(r)) c2r[c[2]] = r; }
    });
    if (Object.keys(c2r).length < 50) throw new Error("World.tsv looked empty");
    _liveElo = c2r; _liveEloAt = Date.now();
    return c2r;
  } catch (e) { return _liveElo; }   // keep last good map; null -> pure static fallback
}
function eloFor(name, live){
  const st = elo.ratings[name];
  const code = ELO_CODE[name];
  if (live && code && live[code] != null) {
    const lv = live[code];
    if (st == null || Math.abs(lv - st) <= ELO_GUARD) return lv;   // accept the fresh value
  }
  return st != null ? st : elo.defaultElo;                         // vetted static / default
}
...
async function buildPayload(opts){
  ...
  const liveElo = await getLiveElo();             // live eloratings.net (cached 6h); null -> static fallback
  ...
  const matches = await Promise.all(picked.matches.map(async m => {
    ...
    const evalOut = model.evaluateMatch({
      homeName, awayName, homeElo: eloFor(homeName, liveElo), awayElo: eloFor(awayName, liveElo),
    ...
    return {
      ...
      home: homeName, away: awayName, homeElo: eloFor(homeName, liveElo), awayElo: eloFor(awayName, liveElo),
  ...
  return {
    ...
    source: { fifa: FIFA_CALENDAR, eloSource: elo._source, sourceError, feedMatches: results.length,
              eloLive: !!liveElo, eloAsOf: _liveEloAt ? new Date(_liveEloAt).toISOString() : null },
    matches
  };
}
