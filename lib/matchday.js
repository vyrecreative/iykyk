...
/* full starting XI with name, shirt, role band (0 GK,1 back,2 mid,3 fwd) and captain */
function lineupArr(team){
  return ((team && team.Players) || []).filter(p => p.Status === 1)
    .map(p => ({ n: desc(p.PlayerName || p.ShortName), shirt: p.ShirtNumber, role: p.Position, captain: !!p.Captain }))
    .filter(x => x.n);
}
