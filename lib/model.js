...
/* In-play scoreline model. lateH/lateA = each side's REAL StatsBomb late-xG share (fraction of its
   ...
   that is NOT ahead, scaled by how late it is and how strong a late-scorer it is. Capped and gentle,
   and only ever applied live - the pre-match price is untouched. Pass nulls to disable (no bump). */
function inPlay(lh, la, minute, hs, as_, rho, lateH, lateA) {
  rho = (rho==null) ? -0.05 : rho;
  const rem = Math.max(0,(94-minute))/90;
  const base = 0.22, K = 1.0, CAP = 1.18;     // base = neutral late share; cap the bump at +18%
  let bh = 1, ba = 1;
  if (minute >= 70) {
    const phase = Math.min(1, Math.max(0, (minute - 70) / 25));   // 0 at 70', ramps to 1 by ~95'
    const fh = Math.min(CAP, 1 + Math.max(0, (lateH||0) - base) * K * phase);
    const fa = Math.min(CAP, 1 + Math.max(0, (lateA||0) - base) * K * phase);
...
function evaluateMatch(opts) {
  ...
  if (opts.live && opts.live.minute != null) {
    live = true;
    probs = inPlay(lh, la, opts.live.minute, opts.live.hs||0, opts.live.as||0, null, opts.lateHome, opts.lateAway);
  } else {
