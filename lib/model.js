...
function inPlay(lh, la, minute, hs, as_, rho) {
  rho = (rho==null) ? -0.05 : rho;
  const rem = Math.max(0,(94-minute))/90;
  const m = matrix(lh*rem, la*rem, rho);
  let h=0,d=0,a=0;
...
function lineupAdjust(teamName, publishedXI, squads) {
  ...
  const eloAdj = -missing.reduce((s,p)=>s+(p.eloImpact||0),0);
...
function evaluateMatch(opts) {
  ...
  if (opts.live && opts.live.minute != null) {
    live = true;
    probs = inPlay(lh, la, opts.live.minute, opts.live.hs||0, opts.live.as||0);
  } else {
